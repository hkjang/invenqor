...
> Invenqor Agent v0.2.19는 Server v0.2.19와 중앙 관리 콘솔을 함께 사용합니다.
...
`invenqor-agents-0.2.19.tar.gz`와 같은 이름의 `.sha256`을 사용할 수 있습니다.
묶음을 한 번 검증·해제한 뒤 대상 장비에는 아키텍처에 맞는 개별 패키지와 그
체크섬만 전달하십시오.

```bash
curl -LO https://github.com/hkjang/invenqor/releases/download/v0.2.19/invenqor-agent-linux-x86_64.tar.gz
curl -LO https://github.com/hkjang/invenqor/releases/download/v0.2.19/invenqor-agent-linux-x86_64.tar.gz.sha256
...
예상 출력은 `invenqor-agent 0.2.19`입니다.
...
```text
invenqor-agent 0.2.19 registration diagnosis at 2026-08-24T09:12:44Z
  host          app-web-01
  ...
  [PASS] server identity               Invenqor Server 0.2.19 (pod invenqor-0, database POSTGRES)
...
```text
invenqor-agent 0.2.19 on app-web-01
...
따라서 Agent를 먼저 올리지 않아도 자동 복구되며, Agent v0.2.19로 순차 업데이트하면
...
확인해 판단하십시오. v0.2.19 내장 카탈로그는 인프라와 보안 제품에 더해
