...
> Invenqor Agent v0.2.11는 Server v0.2.11와 중앙 관리 콘솔을 함께 사용합니다.
...
```bash
curl -LO https://github.com/hkjang/invenqor-agents/releases/download/v0.2.11/invenqor-agent-linux-x86_64.tar.gz
curl -LO https://github.com/hkjang/invenqor-agents/releases/download/v0.2.11/invenqor-agent-linux-x86_64.tar.gz.sha256
...
예상 출력은 `invenqor-agent 0.2.11`입니다.
...
```text
invenqor-agent 0.2.11 registration diagnosis at 2026-07-30T09:12:44Z
  host          app-web-01
  ...
  [PASS] server identity               Invenqor Server 0.2.11 (pod invenqor-0, database POSTGRES)
...
```text
invenqor-agent 0.2.11 on app-web-01
