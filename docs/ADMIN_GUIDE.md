<div class="document-cover">
  ...
  <div class="meta">
    <p><strong>대상 버전</strong> Agent v0.2.11 · Server v0.2.11</p>
...
> Server v0.2.11 운영자는 [Server 설치 및 운영 가이드](SERVER_INSTALLATION.md)를
...
이 문서는 Invenqor Agent v0.2.11를 운영 환경에 배포하는 Linux, 보안, 네트워크,
...
v0.2.11에는 중앙 Server·대시보드·서명 자동 업데이트·자산 API·MCP가 포함됩니다.
...
사용하지만, v0.2.11는 별도 서명 파일이나 공급망 증명(attestation)을 제공하지
...
있지만 v0.2.11 인벤토리 레코드나 전송 envelope에는 포함하지 않습니다.
...
브로드캐스트, VLAN/본딩 관계는 v0.2.11에 포함되지 않습니다.
...
로컬 endpoint를 포함합니다. IPv6 주소는 수집하지만 v0.2.11의 기본 경로 수집은
...
User-Agent: invenqor-agent/0.2.11
...
로그에 관찰만 하며 v0.2.11는 원격 정책이나 명령을 실행하지 않습니다.
...
v0.2.11는 관리자가 승인한 Ed25519 서명 artifact의 자동 스테이징과 권한 분리
...
모두 맞을 때만 `pending.json`을 생성합니다.

설치 직전에 Agent는 스테이징된 바이너리를 실행해 `--version`이 약속한 버전을
보고하는지 확인합니다. 실패하면 설치를 중단하고 기존 바이너리를 유지하므로,
...
2. 오프라인 Ed25519 키로 바이너리 원문을 서명합니다.
   ...
   ```bash
   openssl pkeyutl -sign -rawin -inkey update-signing.pem \
     -in invenqor-agent -out invenqor-agent.sig
   ```

   Server에 `INVENQOR_UPDATE_PUBLIC_KEY`를 설정해 두면 게시 시점에 서명을
   검증하고, 맞지 않으면 `UPDATE_SIGNATURE_REJECTED`로 거부합니다. 설정하지
...
버전을 `allow_downgrade`로 게시합니다. Agent는 그 표시가 있는 릴리즈만 하위
버전으로 받아들이므로, 잘못된 릴리즈에 갇힌 fleet을 중앙에서 꺼낼 수 있습니다.
...
v0.2.11 이후 Agent는 이 상태에서 기본값으로 넘어가지 않고 조치 명령과 함께 기동을
