...
> Server v0.2.34 운영자는 [Server 설치 및 운영 가이드](SERVER_INSTALLATION.md)를
...
이 문서는 Invenqor Agent v0.2.34를 운영 환경에 배포하는 Linux·Windows, 보안, 네트워크,
...
v0.2.34에는 중앙 Server·대시보드·서명 자동 업데이트·자산 API·MCP와 주요
소프트웨어 자동 식별이 포함됩니다.
...
```bash
curl -fLO https://github.com/hkjang/invenqor/releases/download/v0.2.34/invenqor-agent-linux-x86_64.tar.gz
curl -fLO https://github.com/hkjang/invenqor/releases/download/v0.2.34/invenqor-agent-linux-x86_64.tar.gz.sha256
...
`invenqor-agents-0.2.34.tar.gz`와 같은 이름의 `.sha256`을 받으십시오. 이 묶음에는
...
있지만 v0.2.34 인벤토리 레코드나 전송 envelope에는 포함하지 않습니다.
...
브로드캐스트, VLAN/본딩 관계는 v0.2.34에 포함되지 않습니다.
...
로컬 endpoint를 포함합니다. IPv6 주소는 수집하지만 v0.2.34의 기본 경로 수집은
...
- **운영체제 이름**: 모든 Windows 11 호스트는 레지스트리 `ProductName`에
  ...
  체계를 공유하므로 교정 대상이 아닙니다. v0.2.34 Agent는 최상위 `os_name`과
...
놓칩니다. v0.2.34 Server는 매 inventory에서 세 원천을 host 단위로 다시 결합해
...
User-Agent: invenqor-agent/0.2.34
...
로그에 관찰만 하며 v0.2.34는 원격 정책이나 명령을 실행하지 않습니다.
...
v0.2.34 Server는 실패 상태를 기록하는 UPSERT에서 기존 `processed` 행을 제외합니다.
...
v0.2.34는 관리자가 승인한 Ed25519 manifest v2 서명 artifact의 자동 스테이징,
...
v0.2.34 service의 background checker는 서명된 update를 내려받은 뒤 이 적용 경로를
...
v0.2.34부터 Client Secret은 Pod 로컬 파일이 아니라 공용 PostgreSQL의
...
PVC를 가진 첫 v0.2.34 Pod가 시작하면서 공용 DB로 자동 이관합니다. 두 번째 Pod의
...
### 16.2 Keycloak 세션이 있으면 자동 로그인 (`auto_login`)

Keycloak(또는 같은 realm 을 쓰는 다른 SSO)에 이미 로그인한 사용자가 콘솔을 열면
로그인 화면 없이 바로 본 화면으로 들어가게 하는 설정입니다. **설정 → Keycloak**
의 **Keycloak 세션이 있으면 자동 로그인** 토글(`auto_login`)로 켜며 **기본값은
꺼짐**입니다. 꺼진 설치에서는 아무것도 달라지지 않습니다.

동작:

1. 콘솔은 `/api/v1/auth/me` 가 세션 없음을 답하고 `/api/v1/auth/methods` 의
   `keycloak_auto_login` 이 참일 때, 로그인 화면을 그리기 전에
   `/api/v1/auth/keycloak/start?prompt=none&return_to=<현재 주소>` 로 **최상위
   이동**합니다. 숨은 iframe 을 쓰지 않으므로 서드파티 쿠키가 막힌 브라우저에서도
   동작하고 Keycloak 의 프레임 정책과 무관합니다.
2. `prompt=none` 은 화면을 그리지 않습니다. Keycloak 세션이 있으면 인가 코드가
   바로 돌아와 평소 로그인 흐름으로 이어지고, 사용자는 열었던 자리(`return_to`)로
   돌아갑니다. `return_to` 는 `/` 로 시작하고 `//` 로 시작하지 않는 같은 오리진
   경로만 받으며 그 밖의 값은 `/` 가 됩니다.
3. 세션이 없으면 Keycloak 은 `error=login_required` 로 돌아옵니다. 이것은 실패가
   아니라 평범한 대답이므로 Server 는 진단 로그에 남기지 않고 `/?sso=none` 으로
   보내 로그인 화면을 보여 줍니다.

무한 루프 방지 — 거절 뒤에 다시 시도하면 브라우저가 Keycloak 과 콘솔 사이를
끝없이 오가므로 세 겹으로 막습니다.

| 장치 | 동작 |
|---|---|
| 한 탭 세션에 한 번 | 시도 여부를 `sessionStorage` 에 남깁니다. 거절 뒤 새로고침해도 다시 시도하지 않고, 새 탭에서는 다시 시도합니다. |
| 로그아웃 뒤 억제 | 콘솔에서 로그아웃하면 억제 표시를 남깁니다. 다시 세션이 생기면 지워집니다. |
| 주소의 표시 | 콜백이 거절을 받으면 `/?sso=none` 으로 보냅니다. 브라우저 저장소가 지워졌어도 이 주소에서는 다시 시도하지 않습니다. |

브라우저 저장소를 읽지 못하면(사생활 보호 모드, 사이트 데이터 차단) "이미 시도했다"
로 간주해 시도하지 않습니다. 콜백·오류(`auth_error`)·`/api`·`/v1`·`/health`·
`/mcp`·`/momento` 경로에서는 시도하지 않으며, 이미 세션이 있으면 시도하지 않습니다.

Server 쪽 보호: `auto_login` 이 꺼져 있으면 누가 `?prompt=none` 을 붙여 불러도
조용히 평범한 로그인으로 바꿉니다. 리다이렉트가 생기는 자리는 관리자 설정에만
묶입니다. 거절을 인식하는 것도 이 Server 가 `prompt=none` 으로 시작한 흐름에 한하며,
그 `state` 는 한 번 쓰고 폐기됩니다. 평범한 로그인의 `login_required` 나
`access_denied` 같은 실제 오류는 전과 같이 `KEYCLOAK_PROVIDER_REJECTED` 로
기록됩니다.

확인 방법: Keycloak 에 로그인한 브라우저에서 콘솔을 열면 로그인 화면 없이 본
화면이 뜨고, 로그인하지 않은 브라우저에서는 로그인 화면이 한 번 뜨고 새로고침을
반복해도 깜빡이지 않으며, 로그아웃한 뒤 다시 열어도 자동으로 로그인되지 않아야
합니다. 로그인 화면의 **Keycloak으로 계속** 버튼은 설정과 무관하게 평범한 로그인을
시작합니다.

### 16.3 API Key scope·회전 충돌 처리
