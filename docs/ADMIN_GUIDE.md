...
> Server v0.2.35 운영자는 [Server 설치 및 운영 가이드](SERVER_INSTALLATION.md)를
...
이 문서는 Invenqor Agent v0.2.35를 운영 환경에 배포하는 Linux·Windows, 보안, 네트워크,
...
v0.2.35에는 중앙 Server·대시보드·서명 자동 업데이트·자산 API·MCP와 주요
소프트웨어 자동 식별이 포함됩니다.
...
```bash
curl -fLO https://github.com/hkjang/invenqor/releases/download/v0.2.35/invenqor-agent-linux-x86_64.tar.gz
curl -fLO https://github.com/hkjang/invenqor/releases/download/v0.2.35/invenqor-agent-linux-x86_64.tar.gz.sha256
...
`invenqor-agents-0.2.35.tar.gz`와 같은 이름의 `.sha256`을 받으십시오. 이 묶음에는
...
있지만 v0.2.35 인벤토리 레코드나 전송 envelope에는 포함하지 않습니다.
...
브로드캐스트, VLAN/본딩 관계는 v0.2.35에 포함되지 않습니다.
...
로컬 endpoint를 포함합니다. IPv6 주소는 수집하지만 v0.2.35의 기본 경로 수집은
...
- **운영체제 이름**: 모든 Windows 11 호스트는 레지스트리 `ProductName`에
  ...
  체계를 공유하므로 교정 대상이 아닙니다. v0.2.35 Agent는 최상위 `os_name`과
...
놓칩니다. v0.2.35 Server는 매 inventory에서 세 원천을 host 단위로 다시 결합해
...
User-Agent: invenqor-agent/0.2.35
...
로그에 관찰만 하며 v0.2.35는 원격 정책이나 명령을 실행하지 않습니다.
...
v0.2.35 Server는 실패 상태를 기록하는 UPSERT에서 기존 `processed` 행을 제외합니다.
...
v0.2.35는 관리자가 승인한 Ed25519 manifest v2 서명 artifact의 자동 스테이징,
...
v0.2.35 service의 background checker는 서명된 update를 내려받은 뒤 이 적용 경로를
...
v0.2.35부터 Client Secret은 Pod 로컬 파일이 아니라 공용 PostgreSQL의
...
PVC를 가진 첫 v0.2.35 Pod가 시작하면서 공용 DB로 자동 이관합니다. 두 번째 Pod의
