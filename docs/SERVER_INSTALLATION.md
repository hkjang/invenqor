...
대상 Server 버전: v0.2.30 · Agent 버전: v0.2.30 · 기준일: 2026-08-24
...
v0.2.30 Server에서 `(Agent ID, Event ID)`가 같은 이벤트의 `processed` 상태는
...
- `invenqor-0.2.30.tar.gz`
- `invenqor-0.2.30.tar.gz.sha256`
...
- `invenqor-agents-0.2.30.tar.gz`
- `invenqor-agents-0.2.30.tar.gz.sha256`
...
```bash
sha256sum -c invenqor-0.2.30.tar.gz.sha256
gzip -t invenqor-0.2.30.tar.gz
docker load < invenqor-0.2.30.tar.gz
docker image inspect invenqor-server:0.2.30 --format '{{.Id}} {{.Architecture}}'
...
```bash
sha256sum -c invenqor-agents-0.2.30.tar.gz.sha256
gzip -t invenqor-agents-0.2.30.tar.gz
tar -xzf invenqor-agents-0.2.30.tar.gz
```
...
```bash
./scripts/build-offline-images.sh 0.2.30
```
...
```bash
curl -fLO https://github.com/hkjang/invenqor/releases/download/v0.2.30/invenqor-agent-linux-x86_64.tar.gz
curl -fLO https://github.com/hkjang/invenqor/releases/download/v0.2.30/invenqor-agent-linux-x86_64.tar.gz.sha256
...
```powershell
$release = 'https://github.com/hkjang/invenqor/releases/download/v0.2.30'
...
#   updates       자동 · 실행 0.2.14 · 대기 0.2.30
...
```bash
# 공식 릴리즈 artifact 검증
sha256sum -c invenqor-0.2.30.tgz.sha256
helm lint invenqor-0.2.30.tgz --strict
...
helm upgrade --install invenqor ./invenqor-0.2.30.tgz \
  --set replicaCount=2 \
...
`ghcr.io/hkjang/invenqor-server:0.2.30`이며 `linux/amd64`와 `linux/arm64`를
...
v0.2.30부터 Keycloak Client Secret은 공용 PostgreSQL의
...
2. 동일한 Master Key와 기존 `bootstrap.enc`가 있는 PVC를 연결한 v0.2.30 Pod를
   하나 먼저 기동합니다.
...
| `/api/v1/system/info` | 인증 전에도 확인 가능한 버전 `0.2.30` |
...
v0.2.30 검증 경로는 실제 PostgreSQL-backed Server와 Agent 컨테이너를 기동해
...
- Windows Agent의 첫 inventory 시각은 있는데 **운영체제 확인 전**: Server와
  Agent 버전을 확인합니다. v0.2.30는 기존 최상위 `os_name`과 새 `os_release`
...
조정합니다. 즉시 확인하려면 Server v0.2.30 기동 후 한 heartbeat 주기를 기다리고,
