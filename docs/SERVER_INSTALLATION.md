...
대상 Server 버전: v0.2.37 · Agent 버전: v0.2.37 · 기준일: 2026-08-24
...
v0.2.37 Server에서 `(Agent ID, Event ID)`가 같은 이벤트의 `processed` 상태는
...
- `invenqor-v0.2.37.tar.gz`
- `invenqor-v0.2.37.tar.gz.sha256`
...
- `invenqor-agents-0.2.37.tar.gz`
- `invenqor-agents-0.2.37.tar.gz.sha256`
...
```bash
sha256sum -c invenqor-v0.2.37.tar.gz.sha256
gzip -t invenqor-v0.2.37.tar.gz
docker load < invenqor-v0.2.37.tar.gz
docker image inspect invenqor:v0.2.37 --format '{{.Id}} {{.Architecture}}'
...
```bash
sha256sum -c invenqor-agents-0.2.37.tar.gz.sha256
gzip -t invenqor-agents-0.2.37.tar.gz
tar -xzf invenqor-agents-0.2.37.tar.gz
```
...
```bash
./scripts/build-offline-images.sh 0.2.37
```
...
#   updates       자동 · 실행 0.2.14 · 대기 0.2.37
...
```bash
# 공식 릴리즈 artifact 검증
sha256sum -c invenqor-0.2.37.tgz.sha256
helm lint invenqor-0.2.37.tgz --strict
...
helm upgrade --install invenqor ./invenqor-0.2.37.tgz \
  --set replicaCount=2 \
...
`ghcr.io/hkjang/invenqor-server:0.2.37`이며 `linux/amd64`와 `linux/arm64`를
...
v0.2.37부터 Keycloak Client Secret은 공용 PostgreSQL의
...
2. 동일한 Master Key와 기존 `bootstrap.enc`가 있는 PVC를 연결한 v0.2.37 Pod를
   하나 먼저 기동합니다.
...
| `/api/v1/system/info` | 인증 전에도 확인 가능한 버전 `0.2.37` |
...
v0.2.37 검증 경로는 실제 PostgreSQL-backed Server와 Agent 컨테이너를 기동해
...
- Windows Agent의 첫 inventory 시각은 있는데 **운영체제 확인 전**: Server와
  Agent 버전을 확인합니다. v0.2.37는 기존 최상위 `os_name`과 새 `os_release`
...
조정합니다. 즉시 확인하려면 Server v0.2.37 기동 후 한 heartbeat 주기를 기다리고,
...
- **CSV 내려받기**는 현재 조건과 같은 결과를 UTF-8 BOM 포함으로 저장하므로
  스프레드시트에서 바로 열립니다. 추출 자체도 `audit.export`로 감사 기록되며, 그
  기록에는 추출물이 완전했는지(`truncated`)가 함께 남습니다.
- 한 번에 내보내는 행 수는 기본 5,000행이고 `limit`으로 최대 50,000행까지
  조정합니다. 한도에 걸려 **잘린** 추출물은 `invenqor-audit-partial.csv`라는
  이름으로 내려가고 응답에 `X-Invenqor-Truncated: true`와
  `X-Invenqor-Row-Limit`이 붙습니다. 두 헤더가 없으면 완전한 추출물입니다.
...
함께 보여줍니다. **CSV 내려받기**는 현재 조건과 동일한 결과를 저장합니다. 한 번에
내보내는 행 수는 기본 10,000행이고 `limit`으로 최대 100,000행까지 조정합니다.
한도에 걸려 **잘린** 추출물은 `invenqor-assets-partial.csv`라는 이름으로 내려가고
응답에 `X-Invenqor-Truncated: true`와 `X-Invenqor-Row-Limit`이 붙습니다.
...
일은 없습니다. 한 번에 돌려주는 자산 수는 기본 100건이고 `limit`으로 최대 500건까지
조정합니다. 응답에는 조건에 맞는 자산 전체 수 `total`과 다음 행이 있는지를
나타내는 `has_more`·`next_offset`이 붙고, `query.execute` 감사 기록에도
`total`·`offset`이 남습니다. 화면은 제목에 `total`을, 오른쪽에 지금 표시 중인
범위를 보여 주며 `limit`보다 많은 결과는 표 아래의 **이전**·**다음** 버튼으로
같은 질의를 이어서 읽습니다. 외부 스크립트가 순회하는 방법은 자산 API·MCP
가이드의 Query DSL 절을 참조하십시오.
