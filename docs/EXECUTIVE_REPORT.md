<div class="document-cover">
  ...
  <h1>임원 보고서</h1>
  <p class="subtitle">Linux 자산 가시성 확보를 위한 v0.1.0 도입 가치, 통제 경계와 단계별 확산 제안</p>
  <div class="meta">
    <p><strong>대상 릴리즈</strong> v0.1.0</p>
    <p><strong>보고서 버전</strong> 1.0</p>
    <p><strong>기준일</strong> 2026-07-28</p>
    <p><strong>의사결정 등급</strong> Pilot 승인 검토</p>
  </div>
</div>

## v0.2.0 중앙 자산 관리 확장 요약
...
Invenqor Agent v0.1.0은 Linux 서버의 운영체제, 하드웨어 자원, 네트워크,
...
- **안전성**: root나 inbound 포트를 요구하지 않으며, 전송 실패 중 데이터를
  로컬 큐에 보존합니다.
- **현재 경계**: 중앙 수집 서버, 대시보드, CVE 판정, 정책 엔진은 제품 범위 밖이며
  별도 구축 또는 연계가 필요합니다.
...
- release 빌드는 HTTPS만 허용
- 사설 CA, 장비별 mTLS PEM, bearer token 지원
- inbound 포트, 원격 셸, 임의 명령 실행 없음
- 프로세스 명령행 기본 미수집
- 설정과 상태 파일의 제한된 권한
...
산출해야 합니다. v0.1.0 자료만으로 절감액이나 ROI를 단정하지 않습니다.

## 6. 위험과 통제 계획

### 6.1 주요 위험

| 위험 | 영향 | v0.1.0 상태 | 필수 통제 |
|---|---|---|---|
| 중앙 시스템 부재 | 수집 데이터 활용 불가 | 범위 밖 | 게이트웨이·저장·조회 계층 구축 |
...
Invenqor Agent v0.1.0은 Linux 자산 가시성의 가장 아래층인 **현장 수집과 신뢰성
