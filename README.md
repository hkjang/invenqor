...
- 계정 잠김·잠금 해제·계정 생성·Agent 배포 중단을 사내 SMTP 릴레이로 알리는
...
CI의 `windows-native-e2e` 잡이 **실제 Windows 러너**에서 릴리즈 ZIP을 풀어
`scripts/e2e-windows.ps1`을 돌립니다 — 수집 주기, 서버 전송, 서비스 설치·진단까지.
이게 1차 방어선입니다. 10개 수집기가 모두 시작하고 모두 끝났는지, 오류를 보고한
수집기가 없는지 확인합니다. 예전에는 카테고리 4개만 확인해서, 실패한 수집기가 있어도
목록을 만족하면 통과했습니다.

다만 `src/windows_sys.rs`는 `cfg(windows)` 아래에서만 컴파일되므로 이 프로젝트를
개발하는 Linux 장비에서는 그 안의 테스트가 실행되지 않고, CI를 한 바퀴 돌리기
전에는 아무것도 확인되지 않습니다. 그 간격에서 두 결함이 릴리즈까지 갔습니다 —
32비트로 선언해 SCM이 잘라버린 서비스 상태 핸들과, 논리 부정 대신 쓰인 비트 보수
연산이 만든 무한 루프입니다. 로컬에서 바로 돌릴 수 있는 2차 검증이 아래입니다.
