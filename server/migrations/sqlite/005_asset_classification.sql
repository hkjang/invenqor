...
ALTER TABLE assets
    ADD COLUMN classification_confidence REAL NOT NULL DEFAULT 0;
ALTER TABLE assets
    ADD COLUMN classification_rules_json TEXT NOT NULL DEFAULT '[]';
ALTER TABLE assets
    ADD COLUMN manual_fields_json TEXT NOT NULL DEFAULT '[]';
ALTER TABLE assets
...
('20000000-0000-0000-0000-000000000005', '파일시스템 볼륨',
 '마운트된 파일시스템을 스토리지 볼륨으로 분류합니다.', 10, 1,
...
('20000000-0000-0000-0000-000000000009', '네트워크 구성',
 '경로·DNS 구성은 호스트의 구성요소로 연결합니다.', 10, 1,
...
('20000000-0000-0000-0000-000000000007', '계정',
 '로컬 계정을 계정 자산으로 분류합니다. 계정 수가 많아 관계는 만들지 않습니다.', 10, 1,
...
('20000000-0000-0000-0000-000000000010', '데이터베이스 엔진',
 '알려진 데이터베이스 엔진을 database 유형으로 승격하고 호스트 관계를 만듭니다.', 40, 1,
...
('20000000-0000-0000-0000-000000000011', '웹·프록시 계층',
 '웹 서버와 리버스 프록시를 web-tier 태그와 함께 서비스로 유지합니다.', 40, 1,
...
('20000000-0000-0000-0000-000000000012', '컨테이너 런타임',
 '컨테이너 런타임과 orchestrator 구성요소를 표시합니다.', 40, 1,
...
('20000000-0000-0000-0000-000000000013', '메시지·큐 계층',
 '메시지 브로커를 middleware 태그와 함께 표시합니다.', 40, 1,
...
('20000000-0000-0000-0000-000000000021', '스테이징 이름 규칙',
 '호스트 이름 토큰에 stg/stage가 있으면 staging으로 봅니다.', 60, 1,
